...
def get_stopwords(source="spacy"):
    ...
    if source == "nltk":
        ...
        raise ValueError(
            "stopword source {} is not implemented. Please select one of the following options: 'spacy', 'nltk', 'stop_words', 'german_plain', 'german_full'".format(
                source
            )
        )
...
def tokenizer(df, method="spacy"):
    ...
    Args:
        df (pandas dataframe): takes a 1d dataframe of sentence
    ...
    if method == "nltk":
        ...
        corpus = [
            [token.text for token in nlp(line) if token.text if len(token.text) > 1]
            for line in data
        ]
        return corpus
    else:
        raise ValueError(
            "method {} is not implemented. Please select one of following options: 'ntlk', 'spacy'"
        )
